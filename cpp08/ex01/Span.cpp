...
Span::Span(Span const &cpy) : _internal_lst(cpy.getLst()), _capacity(cpy.getCapacity()), _size(0)
{ }
...
unsigned int	Span::longestSpan(void) const
{
	...
	return ( *std::max_element(this->_internal_lst.begin(), this->_internal_lst.end())
		- *std::min_element(this->_internal_lst.begin(), this->_internal_lst.end()) );
}
...
unsigned int	Span::shortestSpan(void) const
{
	...
	{
		for (std::list<int>::const_iterator j = this->_internal_lst.begin(); j != this->_internal_lst.end(); j++)
		{
			if (i == j)
				continue;
			if (std::abs(*i - *j) < min)
