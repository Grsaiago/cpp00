...
void	Bureaucrat::signForm(bool couldSign, std::string formName) const
{
	if (couldSign)
		...
		std::cout << this->getName() << " couldn't sign the form '" << formName << "' because their grade wasn't high enough" << std::endl;
	return ;
}
...
void	Bureaucrat::executeForm(AForm const &form)
{
	try {
		...
		std::cout << "Error on executeForm(): " << error.what() << std::endl;
	}
	return ;
}
