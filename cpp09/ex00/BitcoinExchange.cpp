...
bool	validateDate(struct tm _Nullable *time)
{
	...
	lapYear = isLapYear(static_cast<unsigned int>(time->tm_year + 1900));

	if (!(time->tm_mon >= 0 && time->tm_mday <= 11))
		return (false);
	if (!(time->tm_mday >= 1 && time->tm_mday <= monthDays[lapYear][time->tm_mon]))
		return (false);
	return (true);
}
...
bool		parseDbLine(std::string line, struct rateInfo &rateInfo, std::string format)
{
...
bool		parseFileLine(std::string line, struct rateInfo &rateInfo, std::string format)
{
	char		*successParseDate = NULL;
	struct tm	date;
	std::string	str;
	...
	successParseDate = strptime(line.c_str(), format.c_str(), &date);
	if (successParseDate == NULL)
		throw std::logic_error((std::string("Error: bad input => ") + line).c_str());
	str = successParseDate;
	if (str.find('|') == std::string::npos)
		throw std::logic_error((std::string("Error: bad input => ") + line).c_str());
	str = str.substr(str.find_first_not_of(" \t\r\n|"), str.find_last_not_of(" \t\r\n") + 1);
